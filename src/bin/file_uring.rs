...
static DIR_FD: AtomicI32 = AtomicI32::new(AT_FDCWD);

fn init_dir_fd(path: &str) {
    let Ok(dir) = File::open(path) else {
        error!("Could not open directory: {path}! Using CWD instead!");
        return;
    };

    info!("Root dir: {:?}", dir);

    DIR_FD.store(dir.as_raw_fd(), std::sync::atomic::Ordering::Relaxed);
    mem::forget(dir);
}
...
async fn main() {
    ...
    if args.len() < 3 {
        eprintln!("Usage: {} <address> <port> <log_level> <dir>", args[0]);
        std::process::exit(1);
    }
...
async fn handle_client(stream: &mut TcpStream, drained: Vec<u8>) -> io::Result<()> {
    ...
    while path.is_none() {
        ...
        let _ = sync_stream.read_to_end(&mut drained);
...
fn open_file(ring: &mut IoUring, path: &CStr) -> Result<(), ReadFileErr> {
    let openat_e = opcode::OpenAt::new(
        types::Fd(DIR_FD.load(std::sync::atomic::Ordering::Relaxed)),
        path.as_ptr(),
    )
    .flags(O_RDONLY)
    .build()
...
fn add_splice_sqes(
    ...
    while spliced_out < file_size {
        ...
        let _res = ring.submit_and_wait(num_sqe).expect("err");
        for _i in 0..num_sqe {
